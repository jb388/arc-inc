...
```{r clean up 14C data, echo = FALSE}
...
7. Combine data.
```{r combine 14C soil obs data, echo = FALSE}
...
```{r counts for checking plots, echo = FALSE, include = FALSE}
...
* Data from both experiments plotted together, i.e. "treatment" = air-dry/air-dry + storage
...
```{r pre vs inc 14c plot, echo = FALSE}
...
all.14c.pre.inc %>%
  ggplot(., aes(d14c_corr_mean_pre, d14c_corr_mean_inc, color = Type, shape = treat.bi)) +
...
```{r plot 14c by c resp, echo = FALSE}
...
all.14c.sum2 <- mutate(all.14c, ID.per.exp.trt = paste(ID, Period, Experiment, Treatment)) %>%
  ...
  mutate(d14c_corr_se = d14c_corr_sd/n_mean,
         d14c_corr_u = d14c_corr_mean + d14c_corr_se,
         d14c_corr_l = d14c_corr_mean - d14c_corr_se,
         mgCO2.C_gC_se = mgCO2.C_gC_sd/n_mean,
         mgCO2.C_gC_u = mgCO2.C_gC_mean + mgCO2.C_gC_se,
         mgCO2.C_gC_l = mgCO2.C_gC_mean - mgCO2.C_gC_se) 
...
```{r plot 14c by c resp with outlier, echo = FALSE}
# average control and treatment data
