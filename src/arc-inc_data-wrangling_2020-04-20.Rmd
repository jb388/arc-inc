...
Load flux data from air-dry + storage control samples, and convert from "wide" to "long" format so as to match other data.
...
Load flux data from air-dry + storage samples and from air-dry experiment (ctl & treatment), C & N data for all the Exploratories samples (measured in 2011), and soil mass and moisture data for all experiments.
...
Combine and summarize data in long format to calculate respiration rates and plot over time.
...
rewet.ts <- suppressMessages(left_join(rewet.ts, jarinfo.rewet[ , c("ID", "Treatment", "dw_g")], by = c("ID", "Treatment")))

# combine archive data
archive.ts <- rbind(archive.ctl.ts, 
                    archive.arc.ts[ , names(archive.ctl.ts)])
archive.ts$Experiment <- "archive"
...
# for some reason n() doesn't work when included with the summarize_all syntax
# messages are suppressed, as joining factors with different levels
ts.avg <- suppressMessages(left_join(
  na.omit(ts) %>%
...
ggplot(ts.avg, aes(time_d_cmtv_mean, mgCO2.C_gC_d_mean)) +
  ...
  scale_alpha_manual(name = 'Treatment',
                     values = c("air-dry + storage (2011)" = .2, 
                                "control (2011)" = .2, 
                                "control (2019)" = .4, 
                                "air-dry + storage (2019)" = .4)) +
  scale_linetype_manual(name = 'Treatment',
                        values = c("air-dry + storage (2011)" = 'dashed', 
                                   "control (2011)" ='solid',
                                   "control (2019)" = "dashed",
                                   "air-dry + storage (2019)" = "solid")) +
