...
# calc delta delta
all.14c.sum2$atm14c <- ifelse(all.14c.sum2$YearSampled == 2011,
                              Datm[which(Datm$Date == 2011.5), "NHc14"],
                              Datm[which(Datm$Date == 2019.5), "NHc14"])
all.14c.sum2$dd14c <- all.14c.sum2$d14c_corr_mean - all.14c.sum2$atm14c
# all.14c.sum2 %>%
#   filter(Period == "inc") %>%
#   group_by(Experiment, Treatment, Type) %>%
#   summarise(mean = mean(dd14c))
```
...
>*Caption:* Points show data from all three experiments. Points are the mean of laboratory replicates (for replicated samples); error bars are 2x standard error of the mean. Solid line is 1:1, while the dashed and dotted lines show differences of ±20‰ and ±$40‰, equivalent to the decline in atmospheric CO~2~ over 5 and 10 years respectively during the period of 2000 to 2020.
