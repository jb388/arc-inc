...
archive.ctl.ts.wide <- read.csv("../data/derived/arc-storage-t1-flux/arc-inc_xplr_arc-storage-t1-flux_2020-04-27.csv")
...
tme.ctl.duk <- read.csv("../data/derived/arc-tme-duke/arc-tme-duke_2020-04-28.csv")

# Read Sierra Nevada (Jun's) flux data
tme.ctl.flx.sra <- read.csv("../data/derived/arc-tme-sierra/arc-tme-sierra-flux_2020-04-28.csv")

# Read Sierra Nevada (Jun's) soil data
tme.ctl.soil.sra <- read.csv("../data/derived/arc-tme-sierra/arc-tme-sierra-soil_2020-04-28.csv")

# Read Oak Ridge C data
tme.ctl.ornl <- read.csv("../data/derived/arc-tme-ornl/arc-tme-ornl_2020-04-28.csv")

# Read soil mass and moisture info for storage duration treatment samples
jarinfo.tme <- read.xlsx("../data/raw/lab_jena_results-co2_2018-07-30/ArcInc_IncData_2018.xlsx",
                         sheet = 1)
# remove unit row and convert to appropriate types
jarinfo.tme <- type.convert(jarinfo.tme[-1, ])
...
storage.trt.ts <- rbind(read.csv("../data/derived/arc-tme-t2-flux/arc-tme-t2-flux-1_2020-04-27.csv", stringsAsFactors = FALSE),
...
archive.ctl.ts$Treatment <- ifelse(archive.ctl.ts$Experiment == "2011", "control (2011)", "control (storage duration)")
storage.trt.ts$Treatment <- "storage duration"
...
storage.trt.ts <- merge(storage.trt.ts, jarinfo.tme[ , c("Nr.", "dry.soil")], by.x = "SampleName", by.y = "Nr.")
colnames(storage.trt.ts)[which(names(storage.trt.ts) == "dry.soil")] <- "dw_g"

# add experiment col to archive.arc.ts, storage.trt.ts
archive.arc.ts$Experiment <- "2011"
storage.trt.ts$Experiment <- "tme"

# combine archive data and storage duration data
archive.ts <- rbind(archive.ctl.ts[which(archive.ctl.ts$Treatment != "storage duration"), ], 
                    archive.arc.ts[ , names(archive.ctl.ts)])
storage.ts <- rbind(archive.ctl.ts[which(archive.ctl.ts$Treatment == "storage duration"), ],
                    storage.trt.ts[ , names(archive.ctl.ts)])
...
4. Plots of CO~2~ fluxes over time. The final measurement points for a few samples which took >18 days to reach CO~2~ targets are excluded for display reasons. Respiration rates for those samples remained flat. Notes:
* Daily measurements for the pre-incubation period were not taken for the 2011 control samples, leading to an apparent delay in the timing of the rewetting pulse (Fig 1).
```{r CO2 fluxes plots}
ts.avg %>% 
...
* Plot pre-incubation period as average rate for the first four days for comparability (Fig. 2).
```{r CO2 flux plots II}
...
ts.avg2 <- rbind(ts.pre.avg, ts.inc.avg)
ts.avg2 <- ts.avg2[complete.cases(ts.avg2), ]

# add timepoint zero data
ts.avg2 <- lapply(split(ts.avg2, ts.avg2$Treatment), function(x) {
  lapply(split(x, x$Type), tp0.f)
  })
...
ts.epsn.avg <- ts %>%
  select(Experiment, Period, Treatment, SampleName, gC_gS, dw_g, mgCO2.C_gC, time_d, time_d_cmtv)  %>%
  group_by(Experiment, Period, Treatment, SampleName) %>%
...
# extract external data separately
ext.14c.fx <- function(df) {
  df$SampleName <- df$Probe
  df$Treatment <- "air-dry + storage"
  df$Experiment <- "tme"
  df$Period <- "inc"
  return(df)
}
...
ams_results_ls[1:2] <- lapply(ams_results_ls[1:2], function(x) lapply(x, ext.14c.fx))
ext.14c <- bind_rows(lapply(ams_results_ls[1:2], function(x) bind_rows(x)))
...
ext.14c <- ext.14c[ , nms.ams.vars]
colnames(ext.14c) <- c("fm", "fm_err", "d14c", "d14c_err", nms.ams.vars[5:8])
ext.14c$ID <- substr(ext.14c$SampleName,1,nchar(ext.14c$SampleName)-2)
ext.14c$Type <- ifelse(grepl("G", ext.14c$ID), "G", "F")

# merge Exploratories 14C data with template
...
# need to do this for ext.14c data
ext.14c.cn <- merge(ext.14c, ts.epsn.avg, by = c("SampleName", "Period"))
...
* This seems a bit misleading, as it seems to show upward trend in C resp 
...
* Fig. 2 shows treatment effects as means with inferential error bars (2x SE) [Sue and Alison suggest to display this info in tabular form]
...
14. Create template for external ^14^C data from the second part of the air-dry + storage experiment, i.e. samples for which the control incubations were not performed at MPI-BGC.
```{r external template, include = FALSE}

names(all.14c)
ext.template <- data.frame(matrix(nrow = 0, ncol = length(names(all.14c))))
names(ext.template) <- names(all.14c)
```


15. Show the effect of storage duration by plotting the difference between control and treatment 14C as a function of time archived.
```{r storage duration}
